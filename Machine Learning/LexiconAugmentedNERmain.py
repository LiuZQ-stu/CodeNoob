...
from utils.makeData import DataMaker
...
def evaluate(data, model, name):
    ...
    #acc, p, r, f = get_ner_fmeasget_ner_fmeasureure(gold_results, pred_results, data.tagScheme)
    #return speed, acc, p, r, f, pred_results, gazes
    return speed, pred_results, gazes
...
def train(data, save_model_dir, times, seg=True, train_mode='train'):
    ...
    if train_mode == 'full_train' and times:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--status', choices=['train', 'test', 'full_train'], help='update algorithm', default='full_train')
    ...
    parser.add_argument('--train', default="data/goods_train_unk.char")
    parser.add_argument('--dev', default="data/goods_dev_unk.char" )
    parser.add_argument('--test', default="data/goods_test_unk.char")
    ...
    parser.add_argument('--train_target', choices=['goods', 'query'], default='goods')   ##goods of query
    ...
    char_emb = "../CNNNERmodel/data/goods_char_50d.vec"
    bichar_emb = "../CNNNERmodel/data/goods_bichar_50d.vec"
    gaz_file = "../CNNNERmodel/data/jieba_50d_goods.vec"
    '''if train_file == 'data/demo.train.char':
    ...
    elif train_file == 'data/train_goods.char':
        char_emb = "../CNNNERmodel/data/goods_char_50d.vec"
        bichar_emb = "../CNNNERmodel/data/goods_bichar_50d.vec"
        gaz_file = "../CNNNERmodel/data/jieba_50d_goods.vec"
    else:
        char_emb = "../CNNNERmodel/data/query_char_50d.vec"
        bichar_emb = "../CNNNERmodel/data/query_bichar_50d.vec"
        gaz_file = "../CNNNERmodel/data/jieba_50d_query.vec"'''
    ...
    if status == 'train':
        ...
        data.HP_use_count = args.use_count
        data.generate_instance_with_gaz(test_file,'test')
        result = load_model_decode(save_model_dir, data, 'test', gpu, seg)
        with open('mytest','r') as f:
            for res in result:
                f.write(res)
                f.write('\n')
    elif status == 'full_train':
        data = Data()
        data.HP_gpu = gpu
        data.HP_use_char = args.use_char
        data.HP_batch_size = args.batch_size
        ...
        data.use_bert = args.use_bert
        DM = DataMaker()
        length = DM.DataLength()
        scaler = DM.Partition(length)
        boundary = (1, scaler[-1])
        DM.MakeTrainData(boundary)
        DM.MakeDevAndTest(0, -1)
        data_initialization(data, gaz_file, train_file, dev_file, test_file)
        for idx in range(len(scaler)-1):
            print('--' * 7, str(idx+1) + ' / ' + str(len(scaler)-1) + ' train begins ', '--' * 7)
            boundary = (scaler[idx], scaler[idx+1])
            sys.stdout.flush()
            DM.MakeTrainData(boundary)
            DM.MakeDevAndTest(idx, idx+1)
            data.generate_instance_with_gaz(train_file,'train')
            data.generate_instance_with_gaz(dev_file,'dev')
            data.generate_instance_with_gaz(test_file,'test')
            data.build_word_pretrain_emb(char_emb)
            ...
            break
            set_seed(seed_num)
            train(data, save_model_dir, idx, seg, status)
        print('Dumping data')
        with open(save_data_name, 'wb') as f:
            pickle.dump(data, f)
